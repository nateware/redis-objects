...
on _individual_ data structures, like counters, lists, and sets.  This gem provides
a more Rubyish interface to Redis, by mapping {Redis types}[http://code.google.com/p/redis/wiki/CommandReference]
to Ruby objects, via a thin layer over Ezra's +redis+ gem.  It offers several advantages
over the lower-level redis-rb API:

1. Complex data structures are automatically Marshaled
2. Integers are returned as integers, rather than '17'
3. API is more Rubyish and mirrors Ruby stdlib (eg, Sets)
4. Includes higher-level classes, such as Locks, that wrap several calls
...
integrate with ORMs or other classes), or by using +new+ with the type of data
