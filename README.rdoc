...
access a given data structure _in_between_each_line_of_code_, and whether
...
systems that support a "getset" operation, and Redis is one of them
...
* Only allowing one piece of code to resort a whole table at a time
...
* Counters users see (eg, total_users).  These typically increment.
* Counters you base logic on (eg, remaining_slots).  These typically decrement.

The reason you want two counters is you need to change the value *FIRST*, before
checking it (this means it could go negative momentarily).  Again, otherwise
you're open to a race condition.
...
Then:
    ...
    @course = Course.find(1)
    @course.if_remaining_slots(:>, 0) do
      @course.course_students.create!(:student_id => 101)
...
records, but because you will (at least, should) be using this in a fetched-record
